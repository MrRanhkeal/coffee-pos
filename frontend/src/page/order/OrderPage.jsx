...
import handleClickOut from "../../page/pos/PosPage";
...
function OrderPage() {
    ...
    }, [state.txtSearch, filter.from_date, filter.to_date]);
    ...
    const getOrderDetail = async (data) => {
        try {
            ...
            if (res && !res.error) {
                // Map the order details to include calculated total and ensure all fields are present
                const details = (res.list || []).map(item => ({
                    ...item,
                    p_name: item.p_name || 'Unknown Product',
                    ...
                    sugarLevel: Number(item.sugarLevel || 0)
                }));
                    ...
                    columns={[
                        {
                            ...
                            render: (_, data) => (
                                <div>
                                    ...
                                    <div>
                                        {data.p_category_name} {data.p_brand} {data.sugarLevel ? `${data.sugarLevel}%` : ''}
                                    </div>
                                    <div>{data.p_des}</div>
                                </div>
                            ),
                        ...
                        //   render: (value) => <Tag color="blue">{value ? `${value}%` : '-'}</Tag>,
                        // },
                    ]}
                />
            </Modal>
                        ...
                        render: (item, data, index) => (
                            <Space>
                                ...
                                <Button
                                    type="primary"
                                    icon={<MdPrint />}
                                    onClick={() => handleClickOut(data, index)}
                                />
                            </Space>
                        ),
                    },
