exports.getlist = async (req, res) => {
    try {
        ...
        const filtered_brand_name = {};
        for (const category of categories) {
            const categoryValue = category.value;
            if (all_brand_name[categoryValue]) {
                filtered_brand_name[categoryValue] = all_brand_name[categoryValue];
            }
        }

        res.json({
            categories,
            brand_name: filtered_brand_name,
            message: "success"
        });

    } catch (error) {
        // Assuming logErr is a function you've defined elsewhere
